...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col">
        ...
        <div class="flex items-center justify-center w-full transition-opacity opacity-100 duration-750 lg:grow starting:opacity-0">
            <main class="flex max-w-[335px] w-full flex-col-reverse lg:max-w-4xl lg:flex-row bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden">
                <div class="lg:w-1/2 p-6">
                    <h1 class="text-4xl font-bold mb-4">{{ __("Welcome to the Online Voting System") }}</h1>
                    <p class="mb-4 text-lg">{{ __("Participate in elections, view results, and manage your profile.") }}</p>
                    <a href="{{ route('elections.index') }}" class="text-blue-500 hover:underline text-lg">{{ __("View Elections") }}</a>
                </div>
                <div class="lg:w-1/2 p-6 flex items-center justify-center">
                    <img src="{{ asset('images/voting.svg') }}" alt="Voting" class="w-3/4 h-auto">
                </div>
            </main>
        </div>
